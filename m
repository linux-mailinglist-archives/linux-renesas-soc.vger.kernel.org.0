...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 4B8D2172475
	for <lists+linux-renesas-soc@lfdr.de>; Thu, 27 Feb 2020 18:04:39 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729800AbgB0RDl (ORCPT <rfc822;lists+linux-renesas-soc@lfdr.de>);
        Thu, 27 Feb 2020 12:03:41 -0500
Received: from perceval.ideasonboard.com ([213.167.242.64]:48006 "EHLO
        perceval.ideasonboard.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1729398AbgB0RDk (ORCPT
        <rfc822;linux-renesas-soc@vger.kernel.org>);
        Thu, 27 Feb 2020 12:03:40 -0500
Received: from [192.168.0.20] (cpc89242-aztw30-2-0-cust488.18-1.cable.virginm.net [86.31.129.233])
        by perceval.ideasonboard.com (Postfix) with ESMTPSA id CC652B16;
        Thu, 27 Feb 2020 18:03:32 +0100 (CET)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=ideasonboard.com;
        s=mail; t=1582823013;
        bh=jh/oYSd9SFjjBWf1vtk/AvPo3fO96SCAqBbE1bMJAgo=;
        h=Reply-To:Subject:To:Cc:References:From:Date:In-Reply-To:From;
        b=kSqtexS7vNdHZaxmDYsBHjjayCPINET/Wg/tb9Ih4ma/jKqHecYIkd4oan28r2u2F
         05Qn630SPDzOOFrds/VUMPFIe7IkkbHCBj5VJ5PxLGOWCgPUNa7TUXkRqBQ6b78ldV
         ebSjAUGDd9cl6kp9m1G/gYeIQaMn3iHgH0xQ3YV8=
...
References: <20200214103136.12526-1-kieran.bingham+renesas@ideasonboard.com>
 ...
 <20200221075900.GF5023@valkosipuli.retiisi.org.uk>
From:   Kieran Bingham <kieran.bingham+renesas@ideasonboard.com>
Organization: Ideas on Board
Message-ID: <4139f241-2fde-26ad-fe55-dcaeb76ad6cc@ideasonboard.com>
Date:   Thu, 27 Feb 2020 17:03:29 +0000
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101
 Thunderbird/60.9.1
MIME-Version: 1.0
In-Reply-To: <20200221075900.GF5023@valkosipuli.retiisi.org.uk>
...
Thank you for taking the time to review this, it certainly helps get
this code progressing once again.

On 21/02/2020 07:59, Sakari Ailus wrote:
> Hi Kieran,
> 
> Thank you for the update. I have some comments below.
> 
> On Fri, Feb 14, 2020 at 10:31:36AM +0000, Kieran Bingham wrote:
...
>> channel.
> 
> Do you happen to have an example of a sensor driver that would work with
> this driver?

We are currently using the RDACM20, and we are in progress for sharing
this code across with the RDACM21 (which is why they haven't been posted
yet).

Both the RDACM20/21 use a max9271 serialiser, but different sensor
configurations, so Jacopo is working to create a means to allow the
max9271 serialiser to be common.

I believe that will be posted sometime soon from Jacopo when he's got
RDACM21 working happily.

I'm also aware of an AR0231 based camera which Linaro have been working
on based on some of the framework of this driver, so I would expect that
to later be supported also.


I'll work through your comments below, and aim towards a v8 (I've got a
few other changes to come in too)

I'll see if I can synchronise with Jacopo to post a sensor driver with
the next version.
...
> 
> I understand there is no dependency to the VC patchset.
> 
> How does the receiver figure out what it should receive? Do you assume a
> particular receiver driver, for example?
> 
>>
>> v5: (internal release)
...
> 
> What does this do here, actually? What happens if the user attempts to
> access the sensor before this call?

Before this point the i2c-mux works in a per-channel mode. So only the
channel being used to communicate is open, and any write to another
camera/channel will close and reopen.

See:
 - max9286_i2c_mux_select() /* Open a single channel */
 - max9286_i2c_mux_open() /* Open all channels */
 - max9286_i2c_mux_close() /* Close all channels */

This prevents cross-communication between cameras and ensures that the
i2c-traffic across the i2c-muxes is controlled.

When we 'stream on', however we must have all channels open to allow the
synchronisation signals to be communicated between the cameras

 (which I believe are technically 'gpio' but these can only pass if the
'mux' channel is open)
...
> 
> I believe access to cfg_fmt needs to be serialised, by using e.g. a mutex.
> 
...
> 
> This driver wouldn't get any notification on the upstream pixel rate
> changes. Something to think about indeed.

Indeed, we're trying to fix this up now as we need to convey the correct
pixel rate when 4 cameras are used with higher framesizes which requires
a higher bus speed.
...
> 
> The graph ports and entity pads match now, but they're not the same thing.
> 
> I'd add at least comment along the definition it is used for OF as well.
...
Hrm ... how has this ended up all the way up here ... moving somewhere
more appropriate.
...
Ah - because of this - which is unnecessary - (part of multi-device
workaround - removing).
...
This fails to clean up after the _parse_dt() path here too :-(

>> +		return ret;
>> +
>> +	priv->regulator = regulator_get(&client->dev, "poc");
> 
> Why not devm_regulator_get()?

Indeed, should be fine I believe.
Updated.
...
>> +		regulator_disable(priv->regulator);
> 
> Hmm. Isn't poc_enabled always true (and the regulator enabled) here? If
> so, you could disable the regulator unconditionally.

Hrm ... in /this/ variant of the driver yes, but we have to do a
*really* horrible workaround for an issue if we have two of the same
device on the same I2C bus (which of course is the platform
configuration on our boards).

I'll move this to the support patch for two max9286's on the same bus.
...
(Do you have any comments on [0] per chance too?)
...
So - yes, I think it's quite safe to drop it.
